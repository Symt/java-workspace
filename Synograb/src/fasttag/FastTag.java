...
public class FastTag {
	...
	public FastTag(String[] result) {
		this.result = result;
		// Might do more stuff in the future;
	}
	
	public String[] getTags() {
		//TODO: Write implementation to allow string to have proper parts of speech
	...
	public static void main(String[] args) {
		if (args.length == 0) {
			...
			List<String> words = fasttag.util.Tokenizer.wordsToList("The ball rolled down the street.");
			List<String> tags = tag(words);
			for (int i = 0; i < words.size(); i++)
				System.out.println(words.get(i) + "/" + tags.get(i));
		} else {
			List<String> words = fasttag.util.Tokenizer.wordsToList(args[0]);
			List<String> tags = tag(words);
			for (int i = 0; i < words.size(); i++)
				System.out.println(words.get(i) + "/" + tags.get(i));
		}
